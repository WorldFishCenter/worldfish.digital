 ...
 *  `productSlug`, as { focus, nodes: [{slug, name, type}], edges: [{from, type, to}] }.
 *  Returns null when the product has no relationships. Lets the UI show the whole
 *  local ecosystem (multi-hop), not just direct neighbours. */
export function getRelationshipGraph(productSlug) {
    const edges = relationshipsData.relationships || [];
    const adj = new Map();
    const connect = (a, b) => {
    ...
    const nodes = [...component]
        .map((slug) => {
            ...
            const initiative = product.initiative ? getInitiative(product.initiative) : null;
            return {
                slug,
                name: product.name,
...
/** View-ready markers for the countries globe: every country that resolves to a
 *  geographic point (see lib/countryCoordinates). Cross-cutting entries like
 *  "Global" have no coordinates and are omitted. */
export function getCountryMarkers() {
    const themeName = new Map(getThemes().map((theme) => [theme.slug, theme.name]));
    return countriesData.countries
        .map((country) => {
            ...
            return {
                ...
                themes: (country.themeSlugs || []).map((slug) => themeName.get(slug)).filter(Boolean),
...
export function getInitiative(slug) {
    return bySlug(initiativesData.initiatives, slug);
}
